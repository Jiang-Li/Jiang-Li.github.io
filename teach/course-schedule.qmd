...
if len(df) > 0:
    ...
    function exportTableToCSV(filename) {{
        ...
        for (var i = 0; i < rows.length; i++) {{
            ...
            // Add First Term column
            if (i === 0) {{
                // Header row - add column header
                row.push('First Term');
            }} else {{
                // Data row - check if this is a first term course
                var isFirstTerm = false;
                
                // Check multiple ways to detect first term courses
                var firstTermBadge = currentRow.querySelector('.first-term-badge');
                var hasFirstTermClass = currentRow.classList.contains('first-term');
                var containsFirstTermText = currentRow.textContent.includes('First Term');
                
                console.log('Row', i, 'first term checks:');
                console.log('  - Badge found:', firstTermBadge);
                console.log('  - Has first-term class:', hasFirstTermClass);
                console.log('  - Contains First Term text:', containsFirstTermText);
                
                if (firstTermBadge || hasFirstTermClass || containsFirstTermText) {{
                    isFirstTerm = true;
                }}
                
                // Add Yes/No to the last column
                row.push(isFirstTerm ? 'Yes' : 'No');
                console.log('  - Final result:', isFirstTerm ? 'Yes' : 'No');
            }}
